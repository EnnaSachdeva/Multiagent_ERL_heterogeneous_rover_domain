...
parser.add_argument('-evaluate', type=bool, help='to only evaluate the already trained model', default=False)
    ...
    def __init__(self, popnsize):
        ...
        if self.env_choice == 'rover_loose' or self.env_choice == 'rover_heterogeneous' or self.env_choice == 'rover_tight' or self.env_choice == 'rover_trap':  # Rover Domain
            ...
            ######### and then one long range lidar, which gets activated only when required
                ...
                obs = []
                percentage = 40 #800  # fixme: added for comparison purpose, also need to change from 5 to 2
                ...
                coupling_factor = [0 for _ in range(self.num_agent_types)]
                coupling_factor[0] = 1 # for UAV
                coupling_factor[1] = 1 # for truck
...
class Parameters:
    def __init__(self):
        ...
        self.hidden_size = 128 #fixme: changed from 100
        ...
        self.pg_model_save = self.save_foldername + 'pg_models/'
        ...
        if not os.path.exists(self.pg_model_save): os.makedirs(self.pg_model_save)
...
class MERL:
    ...
    def train(self, gen, test_tracker):
        ...
        #Save models periodically # this is the final model, of the champ of the Evo population
        if gen % 20 == 0:
            ...
            for id, pg_actor in enumerate(self.agents):
                for temp_actor in pg_actor.rollout_actor:
                    torch.save(temp_actor.state_dict(), self.args.pg_model_save + str(id) + '_' + self.args.actor_fname + '_pg')
            print("Models Saved")
...
if __name__ == "__main__":
    ...
    for gen in range(1, 10000000000):  # RUN VIRTUALLY FOREVER
        ...
        print('Ep:/Frames', gen, '/', ai.total_frames, 'Popn stat:', mod.list_stat(popn_fits), 'PG_stat:',
              ...
              ) # we print the min, max, mean and std deviation of all the values
        ...
        if gen % 2 == 0 and args.rollout_size > 0:
            print()
