	...
	def __init__(self, args, id):
		...
		for agent_id in range(args.config.num_agents):
			...
			if(self.args.EVALUATE): # LOAD model
				...
				filename = self.args.model_directory + str(
					agent_id) + "_actor_pop10_roll50_envrover_heterogeneous_fire_truck_uav_long_range_lidar_action_" + str(self.args.action_space)+ "_seed" + str(self.args.seed)+"-rewardglobal_pg"


				m = torch.load(filename)
				temp_model = Actor(args.state_dim, args.action_dim, args.hidden_size, policy_type='DeterministicPolicy')
				...
				if args.ps == 'trunk':
					self.rollout_actor.append(
						MultiHeadActor(args.state_dim, args.action_dim, args.hidden_size, args.config.num_agents))
				else:
					if args.algo_name == 'TD3':
						self.rollout_actor.append(
							Actor(args.state_dim, args.action_dim, args.hidden_size, policy_type='DeterministicPolicy'))
					else:
						self.rollout_actor.append(
							Actor(args.state_dim, args.action_dim, args.hidden_size, policy_type='GaussianPolicy'))
